...
def run(request, params):
    ...
    GDBFrontend.config.connect_host = """+json.dumps(config.CONNECT_HOST)+""";
    GDBFrontend.config.connect_port = """+json.dumps(config.CONNECT_PORT)+""";
    GDBFrontend.imports = {};
    ...
    # 自己添加的,用于动态生成 gdbserver
    gdbserver = f"连接到 {config.CONNECT_HOST}:{config.CONNECT_PORT}"
