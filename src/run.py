...
terminal_id = "gf-" + cur_time_str # 目的是为了同时启动多个 terminal_id
...
def argHandler_connect_host(host):
    arg_config["CONNECT_HOST"] = host
    config.CONNECT_HOST = host

def argHandler_connect_port(port):
    arg_config["CONNECT_PORT"] = port
...
def argHandler_help():
    ...
    print("  --chost, -ch:\t\t\t\tConnect gdbserver host")
    print("  --cport, -cp:\t\t\t\tConnect gdbserver port")
    print("")
...
args = [
    ...
    ["--chost", "-ch", argHandler_connect_host, True],
    ["--cport", "-cp", argHandler_connect_port, True]
]
