...
(function($) {
    ...
    methods.init = function (parameters) {
        ...
        $elements.each(function () {
            ...
            data.collabration.enableEnhancedCollabration = function (parameters) {
                ...
                $.ajax({
                    ...
                    success: function (result_json) {
                        if (!result_json.ok) {
                            GDBFrontend.showMessageBox({text: '发生了一个错误。'});
                            console.trace('发生了一个错误。');
                            return;
                        }
            ...
            data.$gdbFrontend_load_connectBtn.on('click.GDBFrontend', function (event) {
                
                var address = GDBFrontend.config.connect_host + ":" + GDBFrontend.config.connect_port;
                ...
                if (
                    ...
                    GDBFrontend.showMessageBox({text: "请提供一个 gdbserver 地址，例如 \"127.0.0.1:1234\"（主机:端口）或一个串口路径，例如 /dev/tty5。"});
                    return;
                }
            ...
            data.init = function () {
                ...
                data.$gdbFrontend_layout_status_port.html('端口: ' + GDBFrontend.config.http_port);
                ...
                var http_port = GDBFrontend.config.http_port;
                ...
                data.debug.socket.addEventListener('close', function (event) {
                    GDBFrontend.verbose('与调试服务器的连接已关闭。');
                    GDBFrontend.showMessageBox({
                        text: '与 GDBFrontend 服务器的连接已关闭！',
                        on_close: function () {
                    ...
                    data.components.fileTabs.files.every(function (_file, _file_i) {
                        ...
                        $.ajax({
                            ...
                            success: function (result_json) {
                                if (result_json.error) {
                                    if (result_json.error.not_exists) {
                                        GDBFrontend.showMessageBox({text: '路径未找到。'});
                                        console.trace("路径未找到。");
                                    } else if (result_json.error.not_permitted) {
                                        GDBFrontend.showMessageBox({text: '访问被拒绝。'});
                                    } else {
            ...
            $gdbFrontend.on('GDBFrontend_debug_mt_blocking.GDBFrontend', function (event, message) {
                GDBFrontend.showMessageBox({text: 
                    'GDB 主线程被阻塞了。（如果你在 GDB shell 中运行了其他东西,比如 shell,你必须终止它才能让 GDBFrontend 正常工作。）<br><br>' +
                    '<b>如果它继续被阻塞,请在 GDB shell 中按 Ctrl + C。</b>'
                });
            });
            ...
            $gdbFrontend.on('GDBFrontend_debug_mt_blocking_with_interrupting.GDBFrontend', function (event, message) {
                GDBFrontend.showMessageBox({text: 
                    'GDB 主线程被阻塞了。（如果你在 GDB shell 中运行了其他东西，比如 shell,你必须终止它才能让 GDBFrontend 正常工作。）<br><br>' +
                    'GDBFrontend 可能会暂时中断进程并继续工作。<br><br>' +
                    '<b>如果它继续被阻塞，请在 GDB shell 中按 Ctrl + C。</b>'
                });
            });
